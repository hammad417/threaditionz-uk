...
- **Motion = subtle and loopable** (it sits _behind_ headline text, so it must not distract):
...
- **Skill:** `higgsfield-generate` (video). _Not_ `product-photoshoot` (image-only).
- **Method:** **image-to-video** — animate a chosen hero **still** so the exact scene,
  lighting, model and product are preserved and only _motion_ is added. Most reliable and
...
- _(Alternative: reuse `public/hero/home-hero.jpg`, but its model looks away — a fresh
  face-forward still reads better as a video focal point. Recommend new.)_
