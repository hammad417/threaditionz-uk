...
// Social icon SVGs, keyed to BRAND.social. A profile renders only when its URL
// is set in lib/brand.ts, so empty handles (e.g. Pinterest/TikTok until live)
// are silently skipped — never a link to a bare platform homepage.
const SOCIAL_ICONS: Record<keyof typeof BRAND.social, React.ReactNode> = {
  instagram: (
  ...
  tiktok: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="currentColor">
      <path d="M16.6 5.82A4.28 4.28 0 0 1 15.54 3h-3.1v12.4a2.59 2.59 0 0 1-2.59 2.5 2.59 2.59 0 0 1-2.59-2.59 2.59 2.59 0 0 1 3.4-2.46V9.69a5.7 5.7 0 0 0-.81-.06 5.69 5.69 0 1 0 5.69 5.69V9.01a7.35 7.35 0 0 0 4.31 1.38V7.28a4.28 4.28 0 0 1-3.25-1.46Z" />
    </svg>
  ),
};
...
function SocialLinks() {
  const profiles = (
    Object.keys(BRAND.social) as (keyof typeof BRAND.social)[]
  ).filter((key) => BRAND.social[key]);
